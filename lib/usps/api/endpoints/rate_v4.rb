...
def rate_v4(options = {})
				...
				def build_rate_v4_request(xml, options = {})
					...
					options[:rate_v4_request][:packages].each do |package|
						xml.tag!("Package", {ID: "#{package[:id]}"}) do
							tag_unless_blank(xml, 'FirstClassMailType', package[:first_class_mail_type])
							xml.tag!('Service', package[:service])
							xml.tag!('ZipOrigination', package[:zip_origination])
							xml.tag!('ZipDestination', package[:zip_destination])
							xml.tag!('Pounds', package[:pounds])
							xml.tag!('Ounces', package[:ounces])
							xml.tag!('Container', package[:container])
							tag_unless_blank(xml, 'Width', package[:width])
							tag_unless_blank(xml, 'Length', package[:length])
							tag_unless_blank(xml, 'Height', package[:height])
							tag_unless_blank(xml,  'Girth', package[:girth])
							tag_unless_blank(xml, 'Value', package[:value])
							tag_unless_blank(xml, 'Machinable', package[:machinable].to_s.capitalize)
							tag_unless_blank(xml, 'AmountToCollect', package[:amount_to_collect])
							xml.tag!('SpecialServices') do
								tag_unless_blank(xml, 'SpecialService', package[:special_services][:special_service])
								tag_unless_blank(xml, 'Content', package[:special_services][:content])
								tag_unless_blank(xml, 'ContentType', package[:special_services][:content_type])
								tag_unless_blank(xml, 'ContentDescription', package[:special_services][:content_description])
								tag_unless_blank(xml, 'GroundOnly', package[:special_services][:ground_only])
								tag_unless_blank(xml, 'SortBy', package[:special_services][:sort_by])
								tag_unless_blank(xml, 'Machinable', package[:special_services][:machinable])
								tag_unless_blank(xml, 'ReturnLocations', package[:special_services][:return_locations])
								tag_unless_blank(xml, 'ReturnServiceInfo', package[:special_services][:return_service_info])
								tag_unless_blank(xml, 'DropOffTime', package[:special_services][:drop_off_time])
								tag_unless_blank(xml, 'ShipDate', package[:special_services][:ship_date])
								tag_unless_blank(xml, 'Option', package[:special_services][:option])
								tag_unless_blank(xml, 'ReturnDimensionalWeight', package[:special_services][:return_dimensional_weight])
								tag_unless_blank(xml, 'TrackingRetentionPeriod', package[:special_services][:tracking_retention_period])
							end if package[:special_services].present?
						end
					end
					xml.target!
				end

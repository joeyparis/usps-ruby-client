				...
				rows.map do |row|
					...
					columns.each_with_index do |k,i|
						if row[i].is_a? String
							row_data[k.to_s.gsub(/[[:space:]]+/, ' ').strip] = row[i].to_s.gsub(/[[:space:]]+/, ' ').strip
						else
				...
				arr.each do |tag|
					if tag[:type].downcase == '(alias)'
						...
						if nesting.length > 0
							...
							dig_set(shape, nesting_with_children, shape.dig(*nesting_with_children).merge({tag[:name] => tag.merge(children: {})}))
						else
							shape[tag[:name]] = tag
						end
					end
			...
			def parse_doc(path)
				...
				data = table_of_contents.map do |section_number, section|
					...
					section.each do |subsection|
						...
						next if !content_node # For section 1
						...
						if subsection[:version] == "#{section_number}.0"
							...
							section_data[:description] = content_node.content.gsub("\n", ' ')
							...
							section_data[:url] = "#{section_data[:signature][0]['Scheme']}#{section_data[:signature][0]['Host']}#{section_data[:signature][0]['Path'].chomp('?')}"
							section_data[:group] = section_data[:signature][0]['API'].sub('API=','').gsub(/[[:space:]]+/, " ").strip
						...
						# 	section_data[:request_descriptions] = table_hash_array_to_shape(standardize_table_hash_array(table_array_to_hash_array(columns, rows)))
							...
							columns, *rows = content_node.search('tr').map do |tr|
								...
								tr.search('td').map do |td|
									...
									if !has_table
										...
										c, *r = table.search('tr').map{|tr| tr.search('td').map{|td| td.content.gsub(/\n\s/, '').strip}}
							...
							section_data[subsection[:title].downcase.gsub(/\s+/, '_').to_sym] = table_hash_array_to_shape(standardize_table_hash_array(table_array_to_hash_array(columns, rows)))
				...
				data.each do |api|
					...
					# begin
						...
						File.write("spec/usps/api/endpoints/#{api[:group].underscore}_spec.rb", rendered_method_spec)
